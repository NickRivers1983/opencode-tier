...
const TIERS = {
  ...
  ultimate: {
    ...
    requiresGo: false,
    agents: {
      orchestrator: 'opencode/deepseek-v4-flash-free', // Main agent, free Zen, unlimited
  ...
  green: {
    ...
    description: 'Full power — all agents on paid Go models. 1.6T Pro for oracle + orchestrator.',
    costRange: '~$0.08/h — $2–5/day (Go paid)',
    requiresGo: true,
    agents: {
      orchestrator: 'opencode-go/deepseek-v4-pro',  // 1.6T params, ~80% SWE-bench, 1M ctx
  ...
  yellow: {
    ...
    description: 'Balanced — 4 paid (orchestrator/oracle/fixer/designer) + 3 free Zen. ~74% cheaper than GREEN.',
    costRange: '~$0.02/h — $0.50–1.50/day (Go paid)',
    requiresGo: true,
    agents: {
      orchestrator: 'opencode-go/deepseek-v4-flash',  // Paid: main agent needs quality
  ...
  orange: {
    ...
    description: 'Economy — only fixer on paid Go. Orchestrator + oracle use free models.',
    costRange: '~$0.02/h — $0.30–1/day (Go paid, fixer only)',
    requiresGo: true,
    agents: {
      orchestrator: 'opencode/deepseek-v4-flash-free', // Free: most frequent, must be zero-cost
 ...
 *
 * v3.0 thresholds:
 *   green  < 60   — stay premium longer when budget allows
 *   yellow < 80   — balanced zone
 *   orange < 90   — strip to fixer-only paid
 *   red    >= 90  — all free survival
 *
 * Hysteresis (anti-oscillation):
 *   When upgrading from a cheaper tier, the threshold is lowered by HYSTERESIS_GAP.
 *   This prevents rapid flip-flopping: once you downgrade, you need a significant
 *   budget improvement before moving back up. Downgrades are always immediate.
 *
 * @param {number} urgency - 0 (no usage) to 100 (at limit)
 * @param {Object} [options]
 * @param {boolean} [options.hasGo] - Whether Go credits are connected
 * @param {string} [options.currentTier] - Current tier key for hysteresis (e.g. 'red')
 * @param {boolean} [options.skipMidTiers] - Auto-switch only RED↔GREEN, skip YELLOW/ORANGE (default false)
 * @returns {{ key: string, tier: TierConfig }}
 */
function selectTierForUrgency(urgency, { hasGo, currentTier, skipMidTiers } = {}) {
  /** Gap required below threshold when re-upgrading (prevents oscillation). */
  const HYSTERESIS_GAP = 20;

  const baseThresholds = { green: 60, yellow: 80, orange: 90 };

  // Index lookup for comparative tier ordering
  ...
  for (const key of TIER_ORDER) {
    ...
    // Skip mid-tiers (yellow, orange) in strict auto-switch mode.
    // User can still manually: opencode-tier orange|yellow
    if (skipMidTiers && (key === 'yellow' || key === 'orange')) continue;

    let threshold = baseThresholds[key] ?? Infinity;

    // Apply hysteresis: if we're currently in a cheaper tier (higher index)
    // and trying to upgrade to a more expensive one (lower index), make it harder
    if (currentIdx >= 0) {
      const targetIdx = TIER_ORDER.indexOf(key);
      if (targetIdx < currentIdx) {
        threshold = Math.max(0, threshold - HYSTERESIS_GAP);
      }
    }
