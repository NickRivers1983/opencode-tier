...
function installSystemd(scriptPath) {
  ...
  const serviceContent = [
    ...
    'Documentation=https://github.com/nriosdev/opencode-tier',
  ...
  // Timer unit (runs every 30 minutes by default)
  const timerContent = [
    '[Unit]',
    'Description=Run opencode-tier every 30 minutes',
    '',
    '[Timer]',
    'OnBootSec=2min',
  ...
  try {
    ...
    return {
      method: 'systemd',
      success: true,
      message: 'systemd user timer installed (checks every 30 min)',
    };
  } catch (err) {
    return {
      method: 'systemd',
...
<dict>
  ...
  try {
    ...
    return {
      method: 'launchd',
      success: true,
      message: 'launchd agent installed (checks every 30 min)',
    };
  } catch (err) {
    return {
      method: 'launchd',
...
function installCron(scriptPath) {
  const nodeBin = process.execPath;
  const cronLine = `*/30 * * * * ${nodeBin} ${scriptPath} auto --yes >/dev/null 2>&1`;
  ...
  try {
    ...
    return {
      method: 'cron',
      success: true,
      message: 'cron job installed (runs every 30 min)',
    };
  } catch (err) {
    return {
      method: 'cron',
    ...
    try {
      ...
      if (out.includes('opencode-tier')) {
        ...
        return {
          installed: true,
          method: 'systemd',
          details: active ? 'active (checking every 30 min)' : 'installed but inactive',
        };
      }
    } catch { /* ignore */ }
  }
